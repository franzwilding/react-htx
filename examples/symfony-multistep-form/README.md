...
Requires PHP 8.2+ and Composer. The example uses the PHP built-in web server
so there's no dependency on `symfony/cli` — though `symfony serve` works too.
...
│   └── services.yaml
├── public/
│   ├── css/shadcn.css           # hand-written shadcn-token CSS, no Tailwind build needed
│   └── index.php
├── src/
│   ├── Controller/
...
2. Register it globally in `config/packages/twig.yaml`:
   ```yaml
   twig:
       form_themes:
           - 'form/shadcn_form_theme.html.twig'
   ```
3. (Optional) Pull in the supplied [`public/css/shadcn.css`](public/css/shadcn.css)
   if you don't already have Tailwind + shadcn installed. The CSS file
   implements just the utilities the form theme uses, with the shadcn HSL
   tokens; swap it out for your real Tailwind build whenever you're ready.
